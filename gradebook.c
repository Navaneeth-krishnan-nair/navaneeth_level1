...
gradebook input_gradebook()
{
	...
	printf("Enter the number of students and the no of weights: ");
	scanf("%d%d", &gb.no_of_students, &gb.no_weights);
...
void output_gradebook(gradebook gb)
{
	...
	{
		printf("\n%s \t %.2f %c",gb.std[i].name_of_student,gb.std[i].avg_score,gb.std[i].grade);
	}
}
